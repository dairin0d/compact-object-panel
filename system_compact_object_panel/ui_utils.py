...
class INFO_OT_messagebox(bpy.types.Operator):
    ...
    # (if I do this, it causes some memory errors)
    bl_label = "Attention!"
...
class NestedLayout:
    ...
    With standard layouts, it's often necessary to invent
    unique names for each sublayout, which most of the
    time just pollute the namespace.
    
    Note: here properties are propagated to sublayouts!
    ...
    def draw(self, context):
        ...
        with layout.row(True)["main"]:
            ...
            with layout.row(True)(enabled=False)["local"]:
        ...
        with layout.fold("Foldable micro-panel", "box"):
            ...
            with layout.fold("Foldable 2"):
                layout.label("label 8")
    """
    
    _sub_names = {"row", "column", "column_flow", "box", "split"}
    ...
    # At first it thought to implement such storage with
    ...
    folds_keyname = "ui_folds" #chr(1) + "ui_folds" + chr(1)
    ...
    def __exit__(self, type, value, traceback):
        ...
        if type == self.ExitSublayout:
            # This is the layout the exit() was requested for;
            # suppress the exception. Otherwise, let it propagate to parent.
...
def register():
    bpy.utils.register_class(INFO_OT_messagebox)
    
    bpy.utils.register_class(NestedLayout.FoldPG)
    setattr(bpy.types.Screen, NestedLayout.folds_keyname,
        bpy.props.CollectionProperty(type=NestedLayout.FoldPG))

def unregister():
    delattr(bpy.types.Screen, NestedLayout.folds_keyname)
    bpy.utils.unregister_class(NestedLayout.FoldPG)
    
    bpy.utils.unregister_class(INFO_OT_messagebox)
